...
resource "null_resource" "download_install_and_zip" {
  provisioner "local-exec" {
    command = <<-EOT
      git clone git@github.com:ainchor-labs/ainchor-labs-code.git . && cd ainchor-labs-code/${var.path}
      pip install -r requirements.txt &&
      zip -r ${local.zip_name} .
    EOT
  }
}

resource "google_storage_bucket_object" "object" {
  depends_on = [null_resource.download_install_and_zip]
  name       = local.zip_name
  bucket     = var.bucket
  source     = "${path.module}/ainchor-labs-code/${var.path}/${local.zip_name}"
  metadata   = {
